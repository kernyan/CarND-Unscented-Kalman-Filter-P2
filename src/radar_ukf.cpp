...
void RadarUKF::Initialize(){
  ...
  Func2 h_in = [](double dt, const VectorXd x_in){
    ...
    Out(1) = atan2(py, px);
    if (fabs(Out(1)) < SMALL_NUMBER && fabs((Out(1))) < SMALL_NUMBER) {
      cout << "Warning: atan2(0,0) in RadarUKF::Initialize encountered\n";

      double Sign = (py < 0) ? -1 : 1;
      Out(1) = (px < 0) ? M_PI*Sign : 0;      
    }

    if (fabs(Out(0)) < SMALL_NUMBER) {
      Out(0) = SMALL_NUMBER; // arbitrary small number
      cout << "Warning: div0 in RadarUKF::Initialize encountered\n";
    }
  ...
  MatrixXd Qt_in = MatrixXd::Constant(3, 3, 0.0);
  Qt_in << 0.09,0     ,0,
           0   ,0.0009,0,
           0   ,0     ,0.09;
